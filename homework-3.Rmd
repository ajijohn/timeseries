...
plot(onoise$t,onoise$V1 ,col="blue",xlab="year",typ="b",ylab=expression(x[t]),main="Ocean  floor ambient noise",cex=0.5)
...
round(phi <- onoise.reg.acf2[2],3)  # 0.762
...
plot.figPortmanteau(Q.zs,quants.4.20,3,main=expression(paste("Portmanteau Tests of Ocean noise {", z[t],"}")))
...
plot(4:20,1-mapply(pchisq,Q.zs,(4:20)-3),typ="p",xlab="h  (lag)",ylab="p-values",xlim=c(1,20),ylim=c(0,.5),col="blue",main=expression(paste("p-values for Portmanteau Tests of Ocean Noise {", z[t],"}")))
...
plot(onoise$t[-1],onoise.zs,col="blue",xlab="year",typ="b",ylab=expression(z[t]),main=expression(paste("AR(1) Residuals ", z[t] == r[t]-hat(phi)*r[t-1])),cex=0.5)
gg <- diff(onoise.zs) > 0
...
* Summary of IID Tests

    - Informal test for IID likely(null true), as handful are outside 95%CI
    - portmanteau test: reject at level alpha =0.05 for h = 4 thru 20 
