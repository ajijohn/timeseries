---
title: "Homework-3 - Stats 519 - Revised"
author: "Aji John"
output:
...
onoise.reg.acfr <- plot.ACFest(onoise$V1, 
                   expression(paste("Sample ACF for Residuals {", r[t],"}")),20)
hs <- 1:20
lines(hs,onoise.reg.acfr[2]^hs)
...
#Housekeeping
onoise.reg.acff <- acf(onoise$V1, lag.max=20, plot=FALSE)$acf
round(phi <- onoise.reg.acff[2],3)  # -0.006 (was -0.012)
onoise.zs <- onoise$V1[-1] - phi*onoise$V1[-N.onoise]

### overhead IV-13

onoise.acf.zs <- acf(onoise.zs, lag.max=20, plot=FALSE)$acf[-1]
Q.zs <- sapply(1:20,function(h) {portmanteau.test.LB(onoise.acf.zs[1:h],length(onoise.zs))})
quants.1.20 <- sapply(1:20,function(dof) {qchisq(0.05,dof,lower=FALSE)})
quants.1.20 <- sapply((1:20)-3,function(dof) {qchisq(0.05,dof,lower=FALSE)})
...
round(results$p.value,2)    # 0.02 (was  0.016
...
round(results$test.stat,2)  #   0.72 ( was 0.06877224
round(results$p.value,2)    #   0.47 ( was 0.9451709


```
P-value - 0.47, so we fail to reject our null hypothesis.

### Runs test

```{r,echo=TRUE,include=TRUE}

(results <- runs.test(onoise$V1))
results$n.runs              # 66
results$n.pos               # 59(62
results$n.neg               # 69 (65
round(results$mu,1)         # 64.6
round(results$var,1)        # 31.4
round(results$sd,1)         #   5.6
round(results$test.stat,1)  # 0.2 (0.2483314) ( old  0.2737371
round(results$p.value,3)    #  0.80 ( old 0.7842867

```

P-value - 0.80, so we fail to reject our null hypothesis.
...
### Apparently bad choice of words for conclusion. I believe you could use a voting of tests. 