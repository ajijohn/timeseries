...
```{r,echo=TRUE,include=TRUE}
par(mfrow=c(1,2))
plot.ACFest(oneset.z , expression(paste("Sample ACF -Desea/Detre {", z[t],"}")))[2]
abline(h=0)

ss.pacf <- acf(oneset.z , lag.max=40, type="partial", plot=FALSE)
xs <- 1:40
ys <- ss.pacf$acf[1:40]
plot(xs,ys,typ="h",xlab="h  (lag)",ylab="PACF",ylim=c(-1,1),col="blue",main="Sample PACF -Deseason/Detre")
points(xs,ys,col="red",cex=0.5)
